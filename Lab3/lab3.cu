...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    int unit = (height*width)/(2560*1024), remain = (height*width)%(2560*1024);
    int start = unit*idx+min(remain, idx);
    int len = unit+(idx < remain);
...
int main(int argc, char** argv) {
    ...
    sobel<<<2560, 1024>>>(device_s, device_t, height, width, channels);
